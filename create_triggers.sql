...
CREATE TRIGGER trigger_insert_nomenclature_if_it_exists
    ...
    FOR EACH ROW
BEGIN
    SET @exists := EXISTS(SELECT nomenclature_id
    ...
    IF @exists THEN
        ...
        END;
    ELSE
        INSERT INTO table_nomenclatures (product_id, amount)
...
CREATE TRIGGER trigger_set_discount_to_receipt_if_total_price_exceeds_50000
    ...
    FOR EACH ROW
BEGIN
    IF !NEW.discount_is_considered THEN
        BEGIN
                    ...
                    IF NEW.total_price > 50000.0 THEN
                        BEGIN
                            ...
                            IF @existed_discount < 15 THEN
                                UPDATE table_receipts
                                SET discount = 15 AND discount_is_considered = TRUE
                                WHERE receipt_id = NEW.receipt_id;
                            ELSE
                                UPDATE table_receipts
                                SET discount = @existed_discount AND discount_is_considered = TRUE
                                WHERE receipt_id = NEW.receipt_id;
                            END IF;
                        END;
                    ELSE
...
CREATE TRIGGER trigger_insert_client
    ...
    FOR EACH ROW
BEGIN
    SET @exists := EXISTS(SELECT client_id
                          FROM table_clients
                          WHERE NEW.e_mail = e_mail
                             OR NEW.phone = phone);
    IF !@exists THEN
